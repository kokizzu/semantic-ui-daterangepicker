 ...
 * @editor: Kiswono Prayogo http://kokizzu.blogspot.com
 */
...
(function( root, factory ) {
	...
	DateRangePicker.prototype = {
		...
		setOptions: function( options, callback ) {
			...
			if( typeof options.ranges==='object' ) {
				for( range in options.ranges ) {
					...
					if( typeof options.ranges[ range ][ 0 ]==='string' ) start = moment( options.ranges[ range ][ 0 ], this.format );
					else start = moment( options.ranges[ range ][ 0 ] );
					
					if( typeof options.ranges[ range ][ 1 ]==='string' ) end = moment( options.ranges[ range ][ 1 ], this.format );
			...
			if( this.singleDatePicker ) {
				...
				if( !this.timePicker ) this.container.find( '.ranges' ).hide();
				else this.container.find( '.ranges .daterangepicker_start_input, .ranges .daterangepicker_end_input' ).hide();
		...
		updateFormInputs: function() {
			...
			var applyBtn = this.container.find( 'button.applyBtn' );
		...
		setCustomDates: function( startDate, endDate ) {
			...
			if( startDate.isAfter( endDate ) ) {
				...
				if( this.maxDate && endDate.isAfter( this.maxDate ) ) endDate = this.maxDate.clone();
			}
		...
		clickDate: function( e ) {
			...
			if( cal.hasClass( 'left' ) ) {
				...
				if( typeof this.dateLimit==='object' ) {
					...
					if( endDate.isAfter( maxDate ) ) endDate = maxDate;
				}
			} else {
				startDate = this.startDate;
			...
			if( this.singleDatePicker && cal.hasClass( 'left' ) ) endDate = startDate.clone();
			else if( this.singleDatePicker && cal.hasClass( 'right' ) ) startDate = endDate.clone();
		...
		updateTime: function( e ) {
			...
			if( this.timePickerSeconds ) second = parseInt( cal.find( '.secondselect' ).val(), 10 );
		...
		updateCalendars: function() {
			H.Trace( arguments );
			var lC = this.leftCalendar, lCm = lC.month;
			lC.calendar = this.buildCalendar( lCm.month(), lCm.year(), lCm.hour(), lCm.minute(), lCm.second(), 'left' );
			var rC = this.rightCalendar, rCm = rC.month;
			rC.calendar = this.buildCalendar( rCm.month(), rCm.year(), rCm.hour(), rCm.minute(), rCm.second(), 'right' );
			this.container.find( '.calendar.left' ).empty().html( this.renderCalendar( lC.calendar, this.startDate, this.minDate, this.maxDate, 'left' ) );
			this.container.find( '.calendar.right' ).empty().html( this.renderCalendar( rC.calendar, this.endDate, this.singleDatePicker ? this.minDate : this.startDate, this.maxDate, 'right' ) );
		...
		renderCalendar: function( calendar, selected, minDate, maxDate, side ) {
			...
			if( !minDate || minDate.isBefore( calendar.firstDay ) ) html += '<th class="prev available"><i class="left arrow icon"></i></th>';
			else html += '<th></th>';
			
			var dateHtml = this.locale.monthNames[ calendar[ 1 ][ 1 ].month() ] + calendar[ 1 ][ 1 ].format( " YYYY" );
			
			if( this.showDropdowns ) dateHtml = this.renderDropdowns( calendar[ 1 ][ 1 ], minDate, maxDate );
			
			html += '<th colspan="5" class="month">' + dateHtml + '</th>';
			if( !maxDate || maxDate.isAfter( calendar.lastDay ) ) html += '<th class="next available"><i class="right arrow icon"></i></th>';
			else html += '<th></th>';
			...
			for( var row = 0; row<6; ++row ) {
				...
				for( var col = 0; col<7; ++col ) {
					...
					if( (minDate && calendar[ row ][ col ].isBefore( minDate, 'day' )) || (maxDate && calendar[ row ][ col ].isAfter( maxDate, 'day' )) ) {
						...
						cname += ' active ';
						if( calendar[ row ][ col ].format( 'YYYY-MM-DD' )==this.startDate.format( 'YYYY-MM-DD' ) ) cname += ' start-date ';
						if( calendar[ row ][ col ].format( 'YYYY-MM-DD' )==this.endDate.format( 'YYYY-MM-DD' ) ) cname += ' end-date ';
			...
			if( this.timePicker ) {
				...
				for( i = start; i<=end; ++i ) {
					if( i<min_hour || i>max_hour ) html += '<option value="' + i + '" disabled="disabled" class="disabled">' + i + '</option>';
					else html += '<option value="' + i + '" ' + (i==selected_hour ? 'selected="selected"' : '') + '">' + i + '</option>';
				}
				...
				for( i = 0; i<60; i += this.timePickerIncrement ) {
					var num = i;
					if( num<10 ) num = '0' + num;
					if( i<min_minute || i>max_minute ) html += '<option value="' + i + '" disabled="disabled" class="disabled">' + num + '</option>';
					else html += '<option value="' + i + '" ' + (i==selected.minute() ? 'selected="selected"' : '') + '">' + num + '</option>';
				}
